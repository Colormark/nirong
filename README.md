# 泥融 - 低代码开发客户端LCDC
你可接触到的低代码开发工具/平台几乎都是面向开发人员的，普通非开发用户几乎无法使用，对于稍微有技术底子的用户学习成本也很高，故开发这个面向普通用户的“无代码”页面开发（设计）工具，其主思路是：技术人员参与将数据和展示组件化，普通用户可以便捷的选用组件并直接生成网页。  
后续开发的重点是：
1. 更低学习成本的页面设计工具；
2. 融合graphQL等特点，设计一个自表述的API体系，使得普通用户在客户端更容易设计数据的展示形式；
